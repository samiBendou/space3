...
export default class Vector6 extends Float64Array implements Vector, Encoder {
  ...
  equal1(u: Vector): boolean {
    ...
    return (
      Math.abs(x - ux) < epsilon &&
      Math.abs(y - uy) < epsilon &&
      Math.abs(z - uz) < epsilon &&
      Math.abs(vx - uvx) < epsilon &&
      Math.abs(vy - uvy) < epsilon &&
      Math.abs(vz - uvz) < epsilon
    );
  }
  ...
  zero1(): boolean {
    ...
    return (
      ...
      vx <= epsilon * Math.max(1.0, vx) &&
      vy <= epsilon * Math.max(1.0, vy) &&
      vz <= epsilon * Math.max(1.0, vz)
    );
  }
