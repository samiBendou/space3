...
describe("Solver Tests", function () {
  let osolver: Solver<Vector6>, gsolver: Solver<Vector6>;
  ...
  beforeEach(() => {
    ...
    osolver = new Solver(
      ...
      odt,
      Vector6.e(5)
    ); // harmonic oscillator d2u/dt2 = -w * u
    gsolver = new Solver(
      (u: Vector6) => {
  ...
  describe("Step", function () {
    ...
    it("should compute constant acceleration step", () => {
      const solution = [];
      for (let i = 0; i < samples; i++) {
  ...
  describe("Solve", () => {
    const tmax = 10;
    it("should compute gravity solve", () =>
      assert3.solved([gsolver.solve(tmax)], g, tol, tmax / gdt));
    it("should compute oscillator solve", () =>
      assert3.solved([osolver.solve(tmax)], o, 50 * tol, tmax / odt));
  });
});
