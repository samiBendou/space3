...
export const trunc = (decimals: number = 0, vector: Vector): Vector =>
  vector.truncc(decimals);
...
export const der1 = (...vectors: Vector[]): Vector[] =>
  vectors
    .map((vector, index) => vector.subc(vectors[Math.max(0, index - 1)]))
    .slice(1);

/** 1-st order derivative of the vectors `[(u1 - u0) / ds, (u2 - u1) / ds, ...]` */
export const der = (ds: number, ...vectors: Vector[]): Vector[] =>
  vectors
    .map((vector, index) => vector.derc(ds, vectors[Math.max(0, index - 1)]))
    .slice(1);
